# Persistence in ricochet

This application demonstrates the use of the `./persistent` directory in ricochet.

ricochet supports file persistence across deployments by providing a directory called `persistent` that is made available to each deployment and each instance. 

If this application has multiple instances supporting it, each of the shiny application's instances will be using and writing to the same exact SQLite database. 
